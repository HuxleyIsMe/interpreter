...
class TreeOfNumbers {
    ...
    addToTree({leftHalf, root, rightHalf}){
        ...
        while(leftHalf.length) {
            let curr = makeNode(leftHalf.pop())
            if(!currentRoot.left){
                currentRoot.left = curr
            }
        }

        currentRoot = this.head.right = makeNode(rightHalf.pop())
        while(rightHalf.length){
            let curr = makeNode(rightHalf.pop())

            if(!currentRoot.right){
                currentRoot.right = curr
                currentRoot = curr
                continue
            }
